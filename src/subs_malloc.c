...
PS_SUBSCRIBE(CAPTURE_AFTER, EVENT_MALLOC, {
    ...
    struct coldt_alloc_entry *e =
        coldt_thread_append(md, COLDTRACE_ALLOC, ev->ret);
    e->size        = (uint64_t)ev->size;
    e->alloc_index = coldt_next_alloc_idx();
...
PS_SUBSCRIBE(CAPTURE_BEFORE, EVENT_REALLOC, {
    ...
    struct coldt_free_entry *e =
        coldt_thread_append(md, COLDTRACE_FREE, ev->ptr);
    e->alloc_index = coldt_next_alloc_idx();
    e->caller      = (uint64_t)(ev->pc);
})
...
PS_SUBSCRIBE(CAPTURE_AFTER, EVENT_POSIX_MEMALIGN, {
    ...
    struct coldt_alloc_entry *e =
        coldt_thread_append(md, COLDTRACE_ALLOC, ev->ptr);
    e->size        = (uint64_t)ev->size;
    e->alloc_index = coldt_next_alloc_idx();
    e->caller      = (uint64_t)ev->pc;
})
