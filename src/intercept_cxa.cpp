...
PS_SUBSCRIBE_EVENT(INTERCEPT_AFTER, EVENT_CXA_GUARD_ACQUIRE, {
...
PS_SUBSCRIBE_EVENT(INTERCEPT_BEFORE, EVENT_CXA_GUARD_RELEASE, {
    cold_thread *th = coldthread_get();
    cold_thread_prepare(th);
...
PS_SUBSCRIBE_EVENT(INTERCEPT_BEFORE, EVENT_CXA_GUARD_ABORT, {
    cold_thread *th = coldthread_get();
    cold_thread_prepare(th);
    ensure(coldtrace_atomic(&th->ct, COLDTRACE_CXA_GUARD_RELEASE, (uint64_t)arg,
                            get_next_atomic_idx()));
