...
PS_SUBSCRIBE(CAPTURE_EVENT, EVENT_THREAD_START, {
    ...
    struct coldtrace_thread_init_entry *e = coldtrace_append(
        md, COLDTRACE_THREAD_START, (void *)REAL(pthread_self));
    e->atomic_index      = get_next_atomic_idx();
...
PS_SUBSCRIBE(CAPTURE_EVENT, EVENT_THREAD_EXIT, {
    struct coldtrace_atomic_entry *e =
        coldtrace_append(md, COLDTRACE_THREAD_EXIT, (void *)REAL(pthread_self));
...
PS_SUBSCRIBE(CAPTURE_EVENT, EVENT_SELF_FINI, {
    if (self_id(md) == MAIN_THREAD) {
        struct coldtrace_atomic_entry *e = coldtrace_append(
...
PS_SUBSCRIBE(CAPTURE_BEFORE, EVENT_THREAD_CREATE,
             { coldtrace_set_create_idx(md, get_next_atomic_idx()); })

PS_SUBSCRIBE(CAPTURE_AFTER, EVENT_THREAD_CREATE, {
    struct pthread_create_event *ev = EVENT_PAYLOAD(ev);
    struct coldtrace_atomic_entry *e =
        coldtrace_append(md, COLDTRACE_THREAD_CREATE, (void *)*ev->thread);
    e->index = coldtrace_get_create_idx(md);
})

PS_SUBSCRIBE(CAPTURE_AFTER, EVENT_THREAD_JOIN, {
    struct pthread_join_event *ev = EVENT_PAYLOAD(ev);
    struct coldtrace_atomic_entry *e =
        coldtrace_append(md, COLDTRACE_THREAD_JOIN, (void *)ev->thread);
    e->index = get_next_atomic_idx();
})
...
PS_SUBSCRIBE(CAPTURE_BEFORE, EVENT_MUTEX_UNLOCK, {
    ...
    struct coldtrace_atomic_entry *e =
        coldtrace_append(md, COLDTRACE_LOCK_RELEASE, (void *)ev->mutex);
    e->index = get_next_atomic_idx();
})
...
PS_SUBSCRIBE(CAPTURE_AFTER, EVENT_MUTEX_LOCK, {
    ...
    if (ev->ret == 0) {
        struct coldtrace_atomic_entry *e =
            coldtrace_append(md, COLDTRACE_LOCK_ACQUIRE, (void *)ev->mutex);
        e->index = get_next_atomic_idx();
    }
})
...
PS_SUBSCRIBE(CAPTURE_BEFORE, EVENT_RWLOCK_UNLOCK, {
    ...
    struct coldtrace_atomic_entry *e =
        coldtrace_append(md, COLDTRACE_RW_LOCK_REL, (void *)ev->lock);
    e->index = get_next_atomic_idx();
})
...
PS_SUBSCRIBE(CAPTURE_AFTER, EVENT_RWLOCK_RDLOCK, {
    ...
    if (ev->ret == 0) {
        struct coldtrace_atomic_entry *e =
            coldtrace_append(md, COLDTRACE_RW_LOCK_ACQ_SHR, (void *)ev->lock);
        e->index = get_next_atomic_idx();
    }
})
...
PS_SUBSCRIBE(CAPTURE_AFTER, EVENT_RWLOCK_WRLOCK, {
    ...
    if (ev->ret == 0) {
        struct coldtrace_atomic_entry *e =
            coldtrace_append(md, COLDTRACE_RW_LOCK_ACQ_EXC, (void *)ev->lock);
        e->index = get_next_atomic_idx();
    }
})
