...
PS_SUBSCRIBE(CAPTURE_EVENT, EVENT_THREAD_START, {
    coldtrace_init_x(md, self_id(md));
    ...
    struct coldtrace_thread_init_entry *e = coldtrace_append_x(
...
PS_SUBSCRIBE(CAPTURE_EVENT, EVENT_THREAD_EXIT, {
    struct coldtrace_atomic_entry *e = coldtrace_append_x(
...
PS_SUBSCRIBE(CAPTURE_AFTER, EVENT_THREAD_CREATE, {
    ...
    struct coldtrace_atomic_entry *e =
        coldtrace_append_x(md, COLDTRACE_THREAD_CREATE, (void *)*ev->thread);
...
PS_SUBSCRIBE(CAPTURE_AFTER, EVENT_THREAD_JOIN, {
    ...
    struct coldtrace_atomic_entry *e =
        coldtrace_append_x(md, COLDTRACE_THREAD_JOIN, (void *)ev->thread);
    e->index = get_next_atomic_idx();
})
...
PS_SUBSCRIBE(CAPTURE_BEFORE, EVENT_MUTEX_UNLOCK, {
    ...
    struct coldtrace_atomic_entry *e =
        coldtrace_append_x(md, COLDTRACE_LOCK_RELEASE, (void *)ev->mutex);
    e->index = get_next_atomic_idx();
})
...
PS_SUBSCRIBE(CAPTURE_AFTER, EVENT_MUTEX_LOCK, {
    ...
    if (ev->ret == 0) {
        struct coldtrace_atomic_entry *e =
            coldtrace_append_x(md, COLDTRACE_LOCK_ACQUIRE, (void *)ev->mutex);
        e->index = get_next_atomic_idx();
    }
})
...
PS_SUBSCRIBE(CAPTURE_BEFORE, EVENT_RWLOCK_UNLOCK, {
    ...
    struct coldtrace_atomic_entry *e =
        coldtrace_append_x(md, COLDTRACE_RW_LOCK_REL, (void *)ev->lock);
    e->index = get_next_atomic_idx();
})
...
PS_SUBSCRIBE(CAPTURE_AFTER, EVENT_RWLOCK_RDLOCK, {
    ...
    if (ev->ret == 0) {
        struct coldtrace_atomic_entry *e =
            coldtrace_append_x(md, COLDTRACE_RW_LOCK_ACQ_SHR, (void *)ev->lock);
        e->index = get_next_atomic_idx();
    }
})
...
PS_SUBSCRIBE(CAPTURE_AFTER, EVENT_RWLOCK_WRLOCK, {
    ...
    if (ev->ret == 0) {
        struct coldtrace_atomic_entry *e =
            coldtrace_append_x(md, COLDTRACE_RW_LOCK_ACQ_EXC, (void *)ev->lock);
        e->index = get_next_atomic_idx();
    }
})
