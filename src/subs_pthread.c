...
PS_SUBSCRIBE(CAPTURE_EVENT, EVENT_THREAD_START, {
    coldt_thread_init(md, self_id(md));
    ...
    struct coldt_thread_init_entry *e = coldt_thread_append(
        md, COLDTRACE_THREAD_START, (void *)REAL(pthread_self));
    e->atomic_index      = coldt_next_atomic_idx();
...
PS_SUBSCRIBE(CAPTURE_EVENT, EVENT_THREAD_EXIT, {
    struct coldt_atomic_entry *e =
        coldt_thread_append(md, COLDTRACE_THREAD_EXIT, (void *)REAL(pthread_self));
    e->index = coldt_next_atomic_idx();
    coldt_thread_fini(md);
})

PS_SUBSCRIBE(CAPTURE_EVENT, EVENT_SELF_FINI, {
    if (self_id(md) == MAIN_THREAD) {
        struct coldt_atomic_entry *e = coldt_thread_append(
...
PS_SUBSCRIBE(CAPTURE_BEFORE, EVENT_THREAD_CREATE,
             { coldt_thread_set_create_idx(md, coldt_next_atomic_idx()); })

PS_SUBSCRIBE(CAPTURE_AFTER, EVENT_THREAD_CREATE, {
    struct pthread_create_event *ev = EVENT_PAYLOAD(ev);
    struct coldt_atomic_entry *e =
        coldt_thread_append(md, COLDTRACE_THREAD_CREATE, (void *)*ev->thread);
    e->index = coldt_thread_get_create_idx(md);
})

PS_SUBSCRIBE(CAPTURE_AFTER, EVENT_THREAD_JOIN, {
    struct pthread_join_event *ev = EVENT_PAYLOAD(ev);
    struct coldt_atomic_entry *e =
        coldt_thread_append(md, COLDTRACE_THREAD_JOIN, (void *)ev->thread);
    e->index = coldt_next_atomic_idx();
})
...
PS_SUBSCRIBE(CAPTURE_BEFORE, EVENT_MUTEX_UNLOCK, {
    ...
    struct coldt_atomic_entry *e =
        coldt_thread_append(md, COLDTRACE_LOCK_RELEASE, (void *)ev->mutex);
    e->index = coldt_next_atomic_idx();
})
...
PS_SUBSCRIBE(CAPTURE_AFTER, EVENT_MUTEX_LOCK, {
    ...
    if (ev->ret == 0) {
        struct coldt_atomic_entry *e =
            coldt_thread_append(md, COLDTRACE_LOCK_ACQUIRE, (void *)ev->mutex);
        e->index = coldt_next_atomic_idx();
    }
})
...
PS_SUBSCRIBE(CAPTURE_BEFORE, EVENT_RWLOCK_UNLOCK, {
    ...
    struct coldt_atomic_entry *e =
        coldt_thread_append(md, COLDTRACE_RW_LOCK_REL, (void *)ev->lock);
    e->index = coldt_next_atomic_idx();
})
...
PS_SUBSCRIBE(CAPTURE_AFTER, EVENT_RWLOCK_RDLOCK, {
    ...
    if (ev->ret == 0) {
        struct coldt_atomic_entry *e =
            coldt_thread_append(md, COLDTRACE_RW_LOCK_ACQ_SHR, (void *)ev->lock);
        e->index = coldt_next_atomic_idx();
    }
})
...
PS_SUBSCRIBE(CAPTURE_AFTER, EVENT_RWLOCK_WRLOCK, {
    ...
    if (ev->ret == 0) {
        struct coldt_atomic_entry *e =
            coldt_thread_append(md, COLDTRACE_RW_LOCK_ACQ_EXC, (void *)ev->lock);
        e->index = coldt_next_atomic_idx();
    }
})
