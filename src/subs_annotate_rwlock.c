...
PS_SUBSCRIBE(CAPTURE_EVENT, EVENT_ANNOTATE_RWLOCK_CREATE, {
    ...
    COLDTRACE_APPEND_OR_RETURN_OK(e, md, COLDTRACE_RW_LOCK_CREATE,
                                  (void *)ev->lock);
...
PS_SUBSCRIBE(CAPTURE_EVENT, EVENT_ANNOTATE_RWLOCK_DESTROY, {
    ...
    struct coldtrace_atomic_entry *e;
    COLDTRACE_APPEND_OR_RETURN_OK(e, md, COLDTRACE_RW_LOCK_DESTROY,
                                  (void *)ev->lock);
    e->atomic_index = coldtrace_next_atomic_idx();
})
...
PS_SUBSCRIBE(CAPTURE_EVENT, EVENT_ANNOTATE_RWLOCK_ACQ, {
    ...
    struct coldtrace_atomic_entry *e;
    COLDTRACE_APPEND_OR_RETURN_OK(
        e, md, ev->is_w ? COLDTRACE_RW_LOCK_ACQ_EXC : COLDTRACE_RW_LOCK_ACQ_SHR,
        (void *)ev->lock);
    e->atomic_index = coldtrace_next_atomic_idx();
})
PS_SUBSCRIBE(CAPTURE_EVENT, EVENT_ANNOTATE_RWLOCK_REL, {
    ...
    struct coldtrace_atomic_entry *e;
    COLDTRACE_APPEND_OR_RETURN_OK(
        e, md, ev->is_w ? COLDTRACE_RW_LOCK_REL_EXC : COLDTRACE_RW_LOCK_REL_SHR,
        (void *)ev->lock);
    e->atomic_index = coldtrace_next_atomic_idx();
})
