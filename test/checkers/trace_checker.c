...
{
    if (iter->size == INVALID_SIZE) {
        return;
    } else if (size != (uint64_t)iter->size) {
        log_fatal("thread=%lu entry=%d size mismatch found=%lu expected=%d",
                  tid, i, size, iter->size);
    } else if (size == (uint64_t)iter->size) {
        log_info("thread=%lu entry=%d size match=%d", tid, i, iter->size);
    }
}
