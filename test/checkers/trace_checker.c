...
uint64_t
iter_alloc_value(struct entry_it it)
{
    return coldtrace_entry_alloc_index(it.buf);
}
...
uint64_t
iter_atomic_value(struct entry_it it)
{
    return coldtrace_entry_atomic_index(it.buf);
}
...
coldtrace_writer_close(void *page, const size_t size, uint64_t tid)
{
    ...
    for (int i = 0; iter_next(it); iter_advance(&it), i++) {
        ...
        uint64_t pos_atomic = iter_atomic_value(it);
