...
#define EVENT_ENTRY(op)             \
    MEMORY_ORDERS(EVENT_ENTRY_HELPER, op)
...
static inline const char* event_type_to_string(event_type evt) {
    switch (evt) {
#define EVENT_CASE(order, op) case EVENT_TYPE(op, order): return #op "_" #order;
